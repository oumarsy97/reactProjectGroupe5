    ...
    import FollowersPopup from "../Post/FollowersPopup";
    ...
    const SidebarRight = () => {
        ...
        const CreatorsToFollow = () => {
            ...
            const { get: getbestActor } = useCrud('actors/getbestActor');

            useEffect(() => {
                const fetchActors = async () => {
                    try {
                        setIsLoading(true);
                        const actorsData = await getbestActor();
            ...
            const displayedActors = actors.slice(0, 2);
                ...
                return (
                    <div className="bg-white rounded-xl shadow-sm p-6">
                        <p className="text-red-500">
                            ...
                            {error.response?.status === 404 ? " L'endpoint n'a pas été trouvé." : ''}
                        </p>
                    </div>
                );
            }
                    ...
                    {displayedActors.map((actor) => (
                        ...
                        >
                            <div className="flex items-center">
                                <img
                                    src={actor.user?.photo || '/api/placeholder/150/150'}
